...
const SecretTerminal: React.FC<SecretTerminalProps> = ({ onClose, graphContext, onMergeData }) => {
  ...
  const [history, setHistory] = useState<ChatMessage[]>([
    { role: 'system', content: 'ROOT ACCESS GRANTED. WELCOME, USER 1337.\nTHE ARCHITECT IS LISTENING...', timestamp: Date.now() }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const contextSummary = `Total Nodes: ${graphContext.nodes.length}. Root: ${graphContext.nodes[0]?.name}. Sample Nodes: ${graphContext.nodes.slice(0, 5).map(n => n.name).join(', ')}`;
      ...
      setHistory(prev => [...prev, { role: 'system', content: 'ERROR: UPLINK FAILED.', timestamp: Date.now() }]);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-2xl h-[80vh] bg-black border border-green-500 shadow-[0_0_50px_rgba(0,255,0,0.2)] flex flex-col font-mono text-sm md:text-base relative overflow-hidden rounded-lg">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 text-green-300 custom-scrollbar">
          {history.map((msg, i) => (
            <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end text-white' : 'items-start'}`}>
