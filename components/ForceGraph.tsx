...
export const ForceGraph: React.FC<ForceGraphProps> = ({ 
  ...
  useEffect(() => {
    ...
    const onNodeMouseDown = (event: MouseEvent, node: GraphNode) => {
        ...
        if (isLinkingMode) {
            tempLinkRef.current = { source: node, screenX: event.clientX, screenY: event.clientY };
        } else {
    ...
    const onMouseUp = (event: MouseEvent) => { 
        if (tempLinkRef.current) {
            ...
            if (target && target.id !== tempLinkRef.current.source.id && onLinkConnect) {
                onLinkConnect(tempLinkRef.current.source, target);
            }
    ...
    const tick = () => {
        ...
        const projectedNodes = data.nodes.map((node, i) => {
            ...
            let finalScale = projected.s * zoom * (node.spawnProgress || 1) * scaleMultiplier; 
        ...
        merge.each(function(d: any) {
             ...
             if (isInventory) {
                 ...
                 // Wireframe Cube
                 g.select(".cube-shape")
                  .attr("d", getCubePath(25))
                  .attr("fill", "rgba(0,5,16,0.8)")
